...
void Texture_InitFromMemory(texture_t* texture, const uint8_t* data, uint32_t width, uint32_t height, uint8_t channel_count, bool linearize, bool gen_mipmaps)
{
	...
	if (gen_mipmaps && data) {
		...
		glTextureParameterf(id, GL_TEXTURE_MAX_ANISOTROPY, fmax(fmin(16.0f, maxAllowAniso), 0.0f));

		glTextureStorage2D(id, (GLsizei)(1 + floorf(log2f((width, height)))), glFormat, width, height);
